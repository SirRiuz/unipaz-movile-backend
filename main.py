# Python
import json

# Libs
from libs.storage import Storage
from modules.calendar.calendar import Calendar
from modules.calendar.filter import Filter
from modules.auth.auth import Auth
from modules.user.user import User
from modules.califications.califications import Califications


credientials = Auth("1096800901", "9duPJ2nDWMCjLQ8").login()
califications = Califications(credentials=credientials)
califications.get_old_califications()

OPTIONS_LIST = califications.options()
califications.get_actual_califications(OPTIONS_LIST)

# changed = User(credentials=credientials).change_password({
#     "old": "1193096338",
#     "password": "root12345",
#     "confirm": "root12345"
# })

# print(changed)
# #credientials = {'SSO_ID': 'v1.2~1~'}
# data = Calendar(credientials)
# print(data.options())
# filter_data = Filter(data=data.get_class(option="80%232022")).filter(
#     week_days=data.get_calendar_days(),
#     day="thursday"
# )
# print(filter_data)
# print(json.dumps(filter_data, indent=2))
