...
class Califications(BaseClient):
    ...
    def __total_signatures(self) -> (list):
        if self.credentials:
            ...
            SIGNATURES_TABLE = response.find("table", {
                "class": "tabla", "sort": "true"})
            if SIGNATURES_TABLE:
    ...
    def get_actual_califications(self, options) -> (list):
        ...
        if self.credentials and options:
            ...
            for id in options:
                ...
                ROOT_TABLE = response.find("div", {
                    "id": "imprimible_706515_" \
                        + "MISNOTASPARCIALES_22125922"})
                MESSEGES = ROOT_TABLE.find_all("span", {
                    "class": "PortletHeading1"})
                if ROOT_TABLE:
                    CALIFICATION_TABLE = ROOT_TABLE.find("table", {
                        "align": "center","border":"1"})
                    CALIFICATIONS = CALIFICATION_TABLE.find_all("tr")
                    CLASS_TITLE = MESSEGES[len(MESSEGES) - 1]
                    CLASS_TITLE = CLASS_TITLE.text.split('-')[1]\
                        .capitalize()
                    ...
                    for item in range(1, len(CALIFICATIONS)):
                        ...
                        CALIFICATION = CALIFICATION if CALIFICATION \
                            else None
                        
                        CALIFICATION = float(CALIFICATION.replace(",", ".")) if \
                            CALIFICATION else None
    ...
    def get_old_califications(self) -> (list):
        ...
        if self.credentials:
            for option in self.__old_options():
                ...
                DATA_TABLE = response.find_all("table", {
                    "cellpadding": "2", "class": "tabla", "border": "1"})
                for table in DATA_TABLE:
                    SEMESTER_CALIFICATIONS = []
                    for i in table:
                        TABLE_DATA = i.find_all("span", 
                        {"class": "PortletText3"})
                        if TABLE_DATA:
                            CALIFICATION = TABLE_DATA[4].text.replace("   ", "")
                            CALIFICATION = CALIFICATION.replace(",", ".")
                            CALIFICATION = float(CALIFICATION)
                            SIGNATURE = TABLE_DATA[0].text.replace("   ", "")\
                                .capitalize()
    ...
    def options(self) -> (list):
        ...
        if self.credentials:
            ...
            if response:
                SCRIPT_OPTIONS = response.find("script",{
                    "language": "Javascript"})
                OPTIONS_LIST = self.__get_options(SCRIPT_OPTIONS)
                SIZE = self.__total_signatures()

                return OPTIONS_LIST[0:SIZE] if \
                    (SIZE and OPTIONS_LIST) else None
