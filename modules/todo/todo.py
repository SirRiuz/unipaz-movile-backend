...
class Todo:
    ...
    def get(self, id: Optional[str] = None) -> (Union[Dict, list]):
        ...
        try:
            TODO = self.DB.todo.find_one({"user": self.user, "_id": ObjectId(id)})
            if TODO:
                return {
                    "id": str(TODO["_id"]),
                    "title": TODO["title"],
    ...
    def create(self, data: Dict[str, str]) -> Dict[str, str]:
        ...
        self.DB.todo.insert_one(
            {"title": data["title"], "notes": data.get("notes", ""), "user": self.user}
        )
        return data

    def delete(self, id):
        self.DB.todo.delete_one({"user": self.user, "_id": ObjectId(id)})
