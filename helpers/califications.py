def get_future_califications(califications: list) -> (list):
    ...
    for item in califications:
        porcentajes.append(eval(item.get("porcentaje").replace("%", "/100")))

        if item.get("calification") != "--":
            cal.append(float(item.get("calification")))
    ...
    if len(cal) == 1:
        ...
        while True:
            end_court = (
                (3.0 - (first_calification * porcentajes[0]))
            ...
            result = (
                first_calification * porcentajes[0]
                + second_calification * porcentajes[1]
                + end_court * porcentajes[2]
            )

            if result == 3.0 and second_calification >= 3.0 and end_court <= 5.0:
                return (
                    {
